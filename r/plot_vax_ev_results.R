...
sig <- rbindlist(purrr::map(tmp, ~.x[term == "vax_ever:factor(AnyCancerPhe)1"][, .(outcome, ref, p_value)][, sig := fifelse(p_value < 0.05, 1, 0)]))
...
rbindlist(list(
  ...
  geom_pointrange(aes(ymin = conf_low, ymax = conf_high), size = 1, position = position_dodge(.5)) +
  # geom_errorbar(aes(ymin = conf_low, ymax = conf_high), size = 1, width = 0.2, position = position_dodge(.5)) +
