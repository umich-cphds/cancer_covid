make_cancer_by_vax_plot <- function(outcome, title, cancer_var = "AnyCancerPhe", env_name = .GlobalEnv, kable_digits = NULL, ref_level = "0") {
  ...
  unvax_mod_text <- glue("logistf({outcome} ~ factor(vax_status) + factor({cancer_var}) + factor(vax_status):factor({cancer_var}) + {paste(adj_sets[['adj3']], collapse = ' + ')}, data = main %>% mutate(vax_status = relevel(factor(vax_status), ref = 'Prior to vaccination'), {cancer_var} = relevel(factor({cancer_var}), ref = '{ref_level}')), control = logistf.control(maxit = 1000))")
  pvax_mod_text <- glue("logistf({outcome} ~ factor(vax_status) + factor({cancer_var}) + factor(vax_status):factor({cancer_var}) + {paste(adj_sets[['adj3']], collapse = ' + ')}, data = main %>% mutate(vax_status = relevel(factor(vax_status), ref = 'After one dose'), {cancer_var} = relevel(factor({cancer_var}), ref = '{ref_level}')), control = logistf.control(maxit = 1000))")
  fvax_mod_text <- glue("logistf({outcome} ~ factor(vax_status) + factor({cancer_var}) + factor(vax_status):factor({cancer_var}) + {paste(adj_sets[['adj3']], collapse = ' + ')}, data = main %>% mutate(vax_status = relevel(factor(vax_status), ref = 'After two doses'), {cancer_var} = relevel(factor({cancer_var}), ref = '{ref_level}')), control = logistf.control(maxit = 1000))")
  bvax_mod_text <- glue("logistf({outcome} ~ factor(vax_status) + factor({cancer_var}) + factor(vax_status):factor({cancer_var}) + {paste(adj_sets[['adj3']], collapse = ' + ')}, data = main %>% mutate(vax_status = relevel(factor(vax_status), ref = 'After booster'), {cancer_var} = relevel(factor({cancer_var}), ref = '{ref_level}')), control = logistf.control(maxit = 1000))")
