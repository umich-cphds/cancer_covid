...
load_icd_phecode_unmatched_data <- function() {

  # icd9 data
  load(file = "/net/junglebook/michiganmedicine/larsf/data/phenomes/MichiganMedicine_Random30k_20200622/MichiganMedicine_Random30k_20200622_ICD9_Phecodes_Birthyears.Rsav")
  diagnoses_ICD9 <- unique(diagnoses_ICD9)[, lexicon := "ICD9"]
  cli::cli_alert_success("ICD-9 data loaded")

  # icd10 data
  load(file = "/net/junglebook/michiganmedicine/larsf/data/phenomes/MichiganMedicine_Random30k_20200622/MichiganMedicine_Random30k_20200622_ICD10_Phecodes_Birthyears.Rsav")
  diagnoses_ICD10 <- unique(diagnoses_ICD10)[, lexicon := "ICD10"]
  cli::cli_alert_success("ICD-10 data loaded. stacking...")
...
load_tested_covariate_data <- function(outvar, tested_phenome_ids, cancer_tested_ids, cohort = "20220101") {
  ...
  PCRcohort[COV19_positiveYN == 1 &
              Deceased_DaysSinceBirth < DaysSinceBirth_First_PositiveTestDiagnosis + 365.25 / 6,
            OutcomeMax := paste0(OutcomeMax, "D")]

  # Hospitalized within 1 month after first pos test
  PCRcohort[COV19_positiveYN == 1 &
              DaysSinceBirth_First_Hospital_Admission < DaysSinceBirth_First_PositiveTestDiagnosis + 365.25 / 12,
            OutcomeMax := paste0(OutcomeMax, "H")]

  # ICU stay within 1 month after first pos test
  PCRcohort[COV19_positiveYN == 1 &
              DaysSinceBirth_First_ICU_Admission < DaysSinceBirth_First_PositiveTestDiagnosis + 365.25 / 12,
...
# load procedure data ----------
load_procedure_data <- function(exclude_ids, cohort = "20220101") {
