make_bar_plot <- function(data_input, chrt_vsn = "20220801") {
  ...
  sum_tab <- data.table(
    ...
    outcome = c(
      ...
      data_input[in_phenome == 1 & ICU == 1 & AnyCancerPhe == 1, .N],
      data_input[in_phenome == 1 & ICU == 1 & AnyCancerPhe == 0, .N],
  ...
  bar_plot <- ggplot(data = sum_tab, aes(x = status, y = Percent)) +
    ...
    ggtitle("COVID-19 outcomes by cancer status") +
  ...
  pdf(file = paste0("objects/", chrt_vsn, "/bar_plot.pdf"), width = 9,  height = 6)
  print(bar_plot)
  dev.off()
